...
class BusinessQueries:
    ...
    @staticmethod
    async def find_all_by_owner(owner_user_id: ObjectId) -> list[dict]:
        """Return all businesses owned by this user (multi-business support)."""
        cursor = BusinessQueries._col().find({"owner_user_id": owner_user_id})
        return await cursor.to_list(length=None)

    @staticmethod
    async def find_by_owner_and_id(business_id: ObjectId, owner_user_id: ObjectId) -> dict | None:
        """Ownership-safe lookup: returns the business only if both _id and owner_user_id match."""
        return await BusinessQueries._col().find_one(
    ...
    @staticmethod
    async def delete_by_id(business_id: ObjectId) -> bool:
        """Hard-delete a business by its ID. Returns True if a document was deleted."""
        result = await BusinessQueries._col().delete_one({"_id": business_id})
        return result.deleted_count > 0

    @staticmethod
    async def delete_by_owner(business_id: ObjectId, owner_user_id: ObjectId) -> bool:
        """
        Ownership-safe delete: requires BOTH _id AND owner_user_id to match.
        Returns True if a document was deleted.
        """
        result = await BusinessQueries._col().delete_one(
            {"_id": business_id, "owner_user_id": owner_user_id}
        )
        return result.deleted_count > 0
