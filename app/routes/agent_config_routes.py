...
router = APIRouter(prefix="/api/products", tags=["Agent Config"])
_controller = AgentConfigController()


@router.post("/{product_id}/agent-config", status_code=201)
...
@router.get("/{product_id}/agent-config")
...
@router.patch("/{product_id}/agent-config")
