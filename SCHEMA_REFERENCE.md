...
```javascript
{
   ...
   website_url: String,              // moved from user_details.company_website
   contact_email: String,
   contact_phone: String,
   logo_url: String,
   timezone: String,
   owner_user_id: ObjectId,          // One owner can have multiple business docs
...
website_pages
{
  ...
  page_title: String,               // <title> from crawl discovery
  status: Enum,                     // "discovered" | "processing" | "completed" | "failed" | "partial"
  // ── Chunk-level tracking (for partial success handling) ────────────
  total_chunks: Number,             // expected chunks from text extraction
  successful_chunks: Number,        // chunks successfully embedded
...
```
**Indexes:** `(business_id, product_id, knowledge_base_id)`(compound unique)
